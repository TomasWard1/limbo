...
function createControlRouter({ store, handlers }) {
  ...
  function handleHealth(method) {
    ...
    // Count only live sessions (pending/ready/active). Terminal sessions
    // (done/error/timeout) stick around in the store until cleanup() reaps
    // them — users would (correctly) expect activeSessions to drop to 0
    // once a wizard completes, even if the terminal session hasn't been
    // garbage-collected yet.
    const activeSessions = store.list().filter((s) => LIVE_STATUSES.has(s.status)).length;
    return {
      status: 200,
      body: { ok: true, activeSessions },
    };
  }
