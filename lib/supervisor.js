...
function createSupervisor(opts) {
  ...
  const publicServer = publicPort !== null
    ? createPublicServer({ port: publicPort, host: publicHost, channels })
    : null;
  ...
  async function shutdown(info) {
    ...
    shutdownPromise = (async () => {
      ...
      // 3. Stop any channel pipelines before closing the public server so
      //    drain logs are bounded and no further events are ingested.
      for (const [name, channel] of Object.entries(channels)) {
        if (channel && typeof channel.onStop === 'function') {
          try { await channel.onStop(); } catch (err) {
            // best-effort — keep shutting down even if a channel hangs
            // eslint-disable-next-line no-console
            console.error(`supervisor: channel ${name} onStop failed:`, err && err.message);
          }
        }
      }

      // 4. Stop the control server and public server (closes TCP listeners).
      try { await server.stop(); } catch { /* best-effort */ }
      if (publicServer) {
        try { await publicServer.stop(); } catch { /* best-effort */ }
      }

      // 5. Resolve the public shutdown observer.
