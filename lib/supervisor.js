 ...
 *   - control server (Unix-domain-socket HTTP server for the control plane)
 ...
 * it launches OpenClaw as a child, exposes the wizard control plane over
 * the bind-mounted socket, and shuts everything down cleanly when either
...
function createSupervisor(opts) {
  ...
  if (typeof socketPath !== 'string' || !socketPath) {
    throw new Error('createSupervisor: socketPath is required');
  }
  ...
  async function shutdown(info) {
    ...
    shutdownPromise = (async () => {
      ...
      // 3. Stop the control server (unlinks socket).
  ...
  return { start, stop, waitForShutdown };
}
