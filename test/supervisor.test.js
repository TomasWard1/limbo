...
test('supervisor passes channels to the public server', async () => {
  const received = [];
  const channels = {
    whatsapp: {
  ...
  const sup = createSupervisor({
    ...
    publicPort: 0,
    channels,
    spawnSetupServerFn: () => makeFakeChild({ pid: 1 }),
    launchOpenclawFn: () => makeFakeChild({ pid: 2 }),
    nodePath: '/fake/bin/node',
    setupServerPath: '/fake/setup-server/server.js',
    idGenerator: () => 'sess_chan',
    tokenGenerator: () => 'tok_chan',
    wizardPortBase: 18901,
  });

  await sup.start();
  try {
    const res = await httpPostJson(sup.publicPort, '/channel/whatsapp', { hello: 'world' });
    assert.equal(res.status, 200);
    // handler runs async, but quickly in this test
...
test('supervisor.stop() calls channel.onStop for each channel that provides one', async () => {
  const stopped = [];
  const channels = {
    whatsapp: {
      onInbound: async () => {},
      onStop: async () => { stopped.push('whatsapp'); },
    },
    other: {
      onInbound: async () => {},
      // no onStop — must not throw
    },
  };
  ...
  const sup = createSupervisor({
    ...
    nodePath: '/fake/bin/node',
    setupServerPath: '/fake/setup-server/server.js',
    idGenerator: () => 'sess_chan2',
    tokenGenerator: () => 'tok_chan2',
    wizardPortBase: 18901,
  });
  ...
  assert.deepStrictEqual(stopped, ['whatsapp']);
});
