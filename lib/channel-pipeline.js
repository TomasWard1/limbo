...
// User-facing message when the LiteLLM virtual key runs out of budget.
// Ends with "please upgrade plan" per product direction.
const BUDGET_EXHAUSTED_MESSAGE =
  "Se te agotó el plan. Para seguir usando Limbo, please upgrade plan.";
  ...
  async function drain() {
    while (queue.length > 0) {
      ...
      try {
        ...
        // Budget exhaustion is a product-visible condition: reply to the
        // user with the upgrade message instead of failing silently.
        if (err && err.code === 'budget_exceeded') {
          logger.warn('channel-pipeline: budget exhausted — sending upgrade message', {
          ...
          try {
            ...
            logger.error('channel-pipeline: failed to deliver upgrade message', {
              messageId: event.messageId,
